...
class Gait(object):
    ...
    def compute_constraint_violation(self, solution, parameters_list, constraints_list, lower_bounds, upper_bounds):
        ...
        constraint_names_sizes = parameters.get_constraint_names(formulation='static_payload')
...
class GaitNonlinear(Gait):
    ...
    def __init__(self, mass, N, dt, payload_masses, slope_deg, conservative_box=True):
        ...
        Args:
            ...
            conservative_box: if true the arm EE box constraints are more conservative and far from robot chest
        """
        print('Gait with Static Lift off point - Only for one step')
        self._box_conservative = conservative_box
        ...
        for k in range(knot_number):
            ...
            # at the time of lift off impose zero CHP acceleration
            if k > 0:
                left_cubic_hp_acc_constraint = constraints.chp_acc_constraint_3D(
                    P_mov_l[u_slice0:u_slice2], DP_mov_l[u_slice0:u_slice2], dt, k
                )

                right_cubic_hp_acc_constraint = constraints.chp_acc_constraint_3D(
                    P_mov_r[u_slice0:u_slice2], DP_mov_r[u_slice0:u_slice2], dt, k
                )

                self._g_string.extend(left_cubic_hp_acc_constraint['name'])
                self._g_string.extend(right_cubic_hp_acc_constraint['name'])
                g.append(left_cubic_hp_acc_constraint['constraint'])
                g.append(right_cubic_hp_acc_constraint['constraint'])
    ...
    def solve(self, x0, contacts, mov_contact_initial, swing_id, swing_tgt, swing_clearance, swing_t, min_f=50):
        ...
        for k in range(self._knot_number):
            ...
            # state bounds
            if k * self._dt == swing_t[0][0]:
                # at the lift off impose zero CoM velocity and acceleration
                state_bounds = constraints.bound_state_variables(x0, [np.concatenate([np.full(3, -cs.inf), np.zeros(6)]),
                                                                      np.concatenate([np.full(3, cs.inf), np.zeros(6)])],
                                                                      k, self._knot_number, final_state)
            else:
                state_bounds = constraints.bound_state_variables(x0, [np.full(9, -cs.inf), np.full(9, cs.inf)],
                                                                 k, self._knot_number, final_state)
            ...
            # Moving contact position bounds - only initial condition
            if k * self._dt == swing_t[0][0]:
                dp_mov_bound = [np.full(3, 0.0), np.full(3, 0.0)]
            else:
                dp_mov_bound = [np.full(3, -cs.inf), np.full(3, cs.inf)]
            ...
            if k > 0:
                ...
                if k * self._dt == swing_t[0][0]:
                    print('-----------lift off-----------')
                    ddp_mov_bound = [np.zeros(3), np.zeros(3)]
                else:
                    ddp_mov_bound = [np.full(3, -cs.inf), np.full(3, cs.inf)]
                gl.append(ddp_mov_bound[0])  # 2 moving contacts spline acc
                gu.append(ddp_mov_bound[1])

                gl.append(ddp_mov_bound[0])
                gu.append(ddp_mov_bound[1])
