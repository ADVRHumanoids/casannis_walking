...
def casannis(int_freq):
    ...
    base_orient_init = rospy.wait_for_message("/cartesian/PelvisRPY/current_reference", PoseStamped, timeout=None)

    # convert quaternion to euler
    initial_quaternion = [base_orient_init.pose.orientation.x,
                          base_orient_init.pose.orientation.y,
                          base_orient_init.pose.orientation.z,
                          base_orient_init.pose.orientation.w]

    initial_euler = Rotation.from_quat(initial_quaternion).as_euler('xyz', degrees=False)
    ...
    dc0 = np.zeros(3)
    ddc0 = np.zeros(3)
    theta0 = np.array(initial_euler)
    dtheta0 = np.zeros(3)
    ddtheta0 = np.zeros(3)
    x0_euler = np.hstack([theta0, dtheta0, ddtheta0])
    ...
    base_orient_msg = PoseStamped()     # message to be published for base orientation
    ...
    # Pelvis orientation published
    base_orient_pub_ = rospy.Publisher('/cartesian/PelvisRPY/reference', PoseStamped, queue_size=10)
    ...
    sol = walk.solve(x0=x0, x0_euler=x0_euler, contacts=contacts, swing_id=[x-1 for x in swing_id], swing_tgt=swing_tgt,
    ...
    for counter in range(N_total):
        ...
        if not rospy.is_shutdown():
            ...
            # base orientation trajectory
            base_orient_msg.pose.orientation.x = interpl['orient_quaternion'][counter][0]
            base_orient_msg.pose.orientation.y = interpl['orient_quaternion'][counter][1]
            base_orient_msg.pose.orientation.z = interpl['orient_quaternion'][counter][2]
            base_orient_msg.pose.orientation.w = interpl['orient_quaternion'][counter][3]
            ...
            base_orient_msg.header.stamp = rospy.Time.now()
            base_orient_pub_.publish(base_orient_msg)
