...
if __name__ == "__main__":
    ...
    # sc 2
    with open('/home/idadiotis/Downloads/analytical_cost_effect/interpl_analytical_sc2.txt', 'rb') as f:
        analytical_plans2 = pickle.load(f)

    with open('/home/idadiotis/Downloads/analytical_cost_effect/interpl_compatible_sc2.txt', 'rb') as f:
        compatible_plans2 = pickle.load(f)

    with open('/home/idadiotis/Downloads/analytical_cost_effect/interpl_nocost_sc2.txt', 'rb') as f:
        nocost_plans2 = pickle.load(f)

    with open('/home/idadiotis/Downloads/analytical_cost_effect/interpl_noconstraint_sc2.txt', 'rb') as f:
    ...
    # general settings
    fontsize_axes = 1.3 * 40
    fontsize_legends = 1.1 * 32
    linewidth = 1.2 * 4
    ticksize = 1.2 * 28
    ...
    for k, quantity_name in enumerate(plt_quantity_strings):
        ...
        for i in range(3):
            ...
            for j in range(plans_num2):
                plt.plot(time2, plans2[j][quantity_name][i], linewidth=1.5 * linewidth, color=colorstyles[j])
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.xlabel('Time [s]', fontsize=fontsize_axes)
            plt.ylabel(coords[i]+' $[m/s^2]$', fontsize=fontsize_axes)
            if i == 1:
                plt.legend(['no constraint, no cost', 'constraint', 'constraint + cost'], fontsize=fontsize_legends)
        plt.suptitle('Arm Accelerations Sc. 2', fontsize=fontsize_axes)
        plt.tight_layout()
